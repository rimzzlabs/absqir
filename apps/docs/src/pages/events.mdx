...
An event can also refuse a check-in made away from the place it belongs to.
That is its own topic, because a browser cannot tell a real satellite fix
from a made-up one, and the rule is built around that limit rather than
around the map. See [Checking where people are](/location).
