...
Approving marks them present or late **by the clock at the refused scan**,
never at the moment of the decision, so nobody is marked late for how long a
report waited. Declining leaves the record as it is. Either way the member
sees the outcome on their own event page.

One report per person per event.
