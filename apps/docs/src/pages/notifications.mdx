...
Any scheduler that can make that call works: a GitHub Actions workflow, a cron
entry on a server, or a systemd timer. Once a minute is plenty, and once every
five minutes still puts the hour reminder inside its window.

A Cloudflare Cron Trigger on the absqir Worker does nothing, because a trigger
calls a `scheduled()` handler and absqir exports only a `fetch` handler. A
second small Worker whose trigger fetches the endpoint does work.

See [Deploy to Cloudflare](/cloudflare) for the whole path.
