...
import { EventClock } from "./_components/session-diagram";
...
The word in the app is **event**. The API, the database and the addresses
still say `session` (`/sessions`, `/my/sessions`, `attendance_session`),
so nothing you linked or scripted breaks. Only the copy changed, because a
signed-in browser is also a session, and one word for two things confused
people.
...
the link from the event page. The link looks like `/e/<session id>`.
...
the pass while it runs, or when the door opens. `GET /api/my/sessions`
