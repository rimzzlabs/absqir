...
Better Auth owns `user`, `event`, `account`, `verification`,
...
- `event` and `event_group` — one moment people are expected,
  and which groups.
- `attendance_record` — one row per person per event: status, method,
  time, note. Absent rows are written when the event closes.
- `event_registration` — who signed up through an event's public page.
...
event id and the current 20-second time window, keyed by a per-event
...
The member's pass is the other direction: an HMAC over the event id and
