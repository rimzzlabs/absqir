...
```
Browser ── /            Astro pages (dashboard shell, onboarding, sign-in)
        ── /api/...     Hono API (directory, groups, onboarding, OpenAPI)
        ── /api/auth    Better Auth (codes, passwords, organizations)
                        └── Postgres
```
...
Everything an organization owns cascades on delete. Sessions, schedules,
attendance records, leave requests, and notifications arrive in the next
phases.

## The rotating QR token

Sessions come in the next phase, and the check-in token is ready for them.
The QR screen will encode a token that is an HMAC-SHA256 over the session id
and the current 20-second time window, keyed by a per-session secret that
never leaves the server. The screen fetches a fresh token when the window
ends, so a photo of the code expires within seconds, and the server accepts
the current window and the one before it. Only a signed-in member can check
in with it, so a shared photo and a borrowed identifier get nobody in.
