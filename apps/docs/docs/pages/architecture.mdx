...
- `schedule` and `schedule_group` — rules that spawn sessions.
- `attendance_session` and `session_group` — one moment people are expected,
  and which groups.
- `attendance_record` — one row per person per session: status, method,
  time, note. Absent rows are written when the session closes.

Everything an organization owns cascades on delete. Leave requests and
notifications arrive in the next phases.
...
[Sessions and check-in](/sessions).
