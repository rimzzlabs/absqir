...
**Sessions** shows a grid of cards, twelve at a time, with **Upcoming**
...
keeps the sessions that expect that group. The tab, the search and the
group stay in the address, so a link opens the same view.

The API answers `GET /api/sessions` with `items` and a `nextCursor`. Send
...
**Home** is a member's front page: a greeting with the reader's own clock,
the session that runs now or comes next with its check-in buttons, the
attendance rate, the sessions attended, the late count, the pending leave
requests, and the next five sessions beside the last five records.
