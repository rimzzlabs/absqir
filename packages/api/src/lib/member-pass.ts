 ...
 * Signed with the session secret, bound to one person and one session, so
...
export async function createPass(params: PassParams): Promise<string> {
  const payload = `${params.sessionId}.${params.personId}`;
...
export function parsePass(code: string): ParsedPass | null {
  ...
  const [sessionId, personId, signature] = parts;
  if (!sessionId || !personId || !signature) return null;

  return { sessionId, personId, signature };
}

export async function verifyPass(secret: string, pass: ParsedPass): Promise<boolean> {
  const expected = await sign(secret, `${pass.sessionId}.${pass.personId}`);
