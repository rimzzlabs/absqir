...
describe("statusOf", () => {
  it("is scheduled before check-in opens", () => {
    expect(statusOf(session, at("2026-09-08T08:49:00Z"))).toBe("scheduled");
  });

  it("is running from opensBefore until the end", () => {
    expect(statusOf(session, at("2026-09-08T08:50:00Z"))).toBe("running");
    expect(statusOf(session, at("2026-09-08T09:59:59Z"))).toBe("running");
  });

  it("is done at the end even before anyone closed it", () => {
    expect(statusOf(session, at("2026-09-08T10:00:00Z"))).toBe("done");
    expect(needsFinalising(session, at("2026-09-08T10:00:00Z"))).toBe(true);
  });

  it("runs early when an organizer opened it", () => {
    const opened = { ...session, openedAt: at("2026-09-08T08:00:00Z") };
    expect(statusOf(opened, at("2026-09-08T08:01:00Z"))).toBe("running");
  });

  it("is done once closed, whatever the clock says", () => {
    const closed = { ...session, closedAt: at("2026-09-08T09:30:00Z") };
